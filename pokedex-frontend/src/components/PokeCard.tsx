...
import {
  Card, CardActionArea, CardContent, CardMedia, Box, Chip, Typography,
        ...
        <Tooltip title={isFav ? 'Remove from My Pokédex' : 'Add to My Pokédex'}>
          <IconButton
            ...
            sx={{
              ...
              '&:hover': { color: 'secondary.main', bgcolor: alpha(theme.palette.secondary.main, 0.1) },
            }}
          >
            {isFav ? <Favorite fontSize="small" /> : <FavoriteBorderOutlined fontSize="small" />}
          </IconButton>
        </Tooltip>

        <CardActionArea onClick={onClick} sx={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', p: 2, pt: 3 }}>
          {/* Pokémon Image */}
          <Box
            sx={{
              width: 120, height: 120,
              borderRadius: '50%',
              background: alpha(primaryColor, theme.palette.mode === 'dark' ? 0.15 : 0.1),
          ...
          >
            <Box
              ...
              alt={pokemon.name}
              loading="lazy"
              sx={{ width: 90, height: 90, objectFit: 'contain', filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.25))' }}
            />
          </Box>

          <CardContent sx={{ p: 0, width: '100%', textAlign: 'center' }}>
            {/* Dex number */}
            <Typography variant="caption" color="text.disabled" fontWeight={700} letterSpacing={2} sx={{ textTransform: 'uppercase' }}>
              #{pokemon.id.toString().padStart(4, '0')}
            </Typography>

            {/* Name */}
            <Typography variant="h6" fontWeight={800} sx={{ textTransform: 'capitalize', my: 0.5, color: 'text.primary', letterSpacing: -0.5 }}>
              {pokemon.name}
            </Typography>

            {/* Types */}
            <Box display="flex" justifyContent="center" gap={0.75} flexWrap="wrap">
